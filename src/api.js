...
var token;
/**
 * è®¾ç½®tokenï¼åæ¶ç¼å­å° localStorage å cookie
 * 
 * @export
 * @param {any} aToken
 */
function setToken(aToken, type) {
    if (true) console.log('[token]setToken:', type, aToken);
    token = aToken;
    if (aToken) {
...
function delToken(type) {
    if (true) console.log('[token]delToken:', type);
...
/**
 * åå§åtoken
 * - å¨ localStorage å cookie ä¸­æ¥æ¾ token
 * - token å­å¨ï¼è°ç¨ setToken
 * 
 * @export
 * @returns æ¯å¦æ token
 */
function initial(type) {
    if (true) console.log('[token]initialToken:', type);
    setToken(localStorage.getItem('t' + type) || CookieUtil.get('t' + type), type);
    return getToken();
}
 ...
 * @param {string|FormData} data è¯·æ±ä½
 * @returns å·²æåºåºå±éè¯¯ï¼åæ¬4xxï¼5xxï¼ï¼éè¦å¨then()çåè°ä¸­åæå¹¶æåºèªå®ä¹éè¯¯
 */
 ...
 * 
 * @export
 * @param {string} uid å­¦çå­¦å·ï¼è¾å¯¼åå¯¼å¥è´¦å·
 * @param {string} pswd å¯ç 
 * @param {string} type ç¨æ·ç±»åï¼åªè½ä¸º guide/student
 */
...
/* harmony export (immutable) */ exports["getBuildings"] = getBuildings;
/* harmony export (immutable) */ exports["getRooms"] = getRooms;
/**
 * è¾å¯¼åç¨æ·api
 * @module api-guide
 * @author initial-wu
...
/**
 * ä¸å¡apiæç»è¿åçPromiseçthenåè°
 * @callback finalThenCallback
 * @param {Object} data ä¸å¡æ°æ®
 */

/**
 * ä¸å¡apiæç»è¿åçPromiseçcatchåè°
 * @callback finalCatchCallback
 * @param {Error} err éè¯¯ï¼åå«éè¯¯ä¿¡æ¯
 * @param {string} err.message éè¯¯ä¿¡æ¯
 */

/**
 * httpä¸­é´ä»¶è¿åçPromiseçcatchåè°
 * @callback middleCatchCallback
 * @param {Object} res httpè¿å
 * @param {number} res.status ç¶æç 
 * @param {Object} res.data ä¸å¡æ°æ®
 */
...
/**
 * httpä¸­é´ä»¶
 * - å¨è¯·æ±æ°æ®ä¸­æ··å¥token
 * - æè·è¿åæ°æ®ä¸­çå¨å±éè¯¯
 * 
 * @param {string} path è·¯å¾
 * @param {Object} params åæ°åè£æå¯¹è±¡
 * @returns  Promise éè¦å¨then()çåè°ä¸­åæå¹¶æåºä¸å¡çº§éè¯¯
 */
 ...
 * 
 * @param {string} path è·¯å¾
 * @param {FormData} formData åæ°åè£æFormData
 * @returns  Promise éè¦å¨then()çåè°ä¸­åæå¹¶æåºä¸å¡çº§éè¯¯
 */
...
/**
 * éè¿å­¦å·åºé´æç´¢å­¦ç
 * 
 * @export
 * @param {string} bound - åªè½ä¸ºbound/unbound
 * @param {string} beginUid å¼å§å­¦å·ï¼å¯ä»¥ä¸å®æ´
 * @param {string} endUid ç»æå­¦å·ï¼å¯ä»¥ä¸å®æ´
 * @param {number} pageNum é¡µç ï¼ä» 1 å¼å§
 * @param {number} pageSize é¡µé¢å¤§å°ï¼æå°ä¸º 1
 * @returns {Function} promise.catch ä¼ å¥åè° catch(function(err){}) err.message:éè¯¯ä¿¡æ¯
 * @returns {Function} promise.then ä¼ å¥åè° then(function(data){}) data.total:æ»æ°ï¼data.list:å­¦çæ°ç»
 */
 ...
 * 
 * @export
 * @param {FormData} formData åå«excelæä»¶
 */
 ...
 * 
 * @export
 * @param {string} state ç¶æï¼åªè½ä¸ºall/pending/approved/forbidden/finished
 * @param {Object} limits éå¶æ¡ä»¶
 * @param {string} limits.type åå«ï¼åªè½ä¸ºaffair/sick/meeting/practice
 * @param {string} limits.out æ¯å¦ç¦»æ ¡ï¼åªè½ä¸ºTRUE/FALSE
 * @param {number} pageNum é¡µç ï¼ä» 1 å¼å§
 * @param {number} pageSize é¡µé¢å¤§å°ï¼æå°ä¸º 1
 */
...
/**
 * æ ¹æ®åæ¡ lid ä¿®æ¹åæ¡ç¶æ
 * 
 * @export
 * @param {string} lid åæ¡id
 * @param {string} state ç¶æï¼åªè½ä¸ºapproved/forbidden/finished
 */
...
/**
 * æ ¹æ® sfid è·åæªç­¾å°å­¦çç uid æ°ç»
 * 
 * @export
 * @param {string} sfid signForm id
 */
...
function getBuildings(schoolZone) {
    var path = '/public/getBuildings.do';
    return postJSON(path, {
...
/* harmony export (immutable) */ exports["getBuildings"] = getBuildings;
/* harmony export (immutable) */ exports["getRooms"] = getRooms;
/* harmony export (immutable) */ exports["getSignature"] = getSignature;
/**
 * è¾å¯¼åç¨æ·api
 * @module api-student
 * @author initial-wu
...
/**
 * æ°åæ¡ææ´æ¹åæ¡
 * - æ´æ¹åæ¡æ¶noteéåå«lid
 * - åªè½å¯¹æªå¤ççåæ¡è°ç¨
 * - è¿åæ°åæ¡
 * @param {Object} note - åæ¡å¯¹è±¡
 */
...
/**
 * æ¤éè¯·åç³è¯·ï¼åªè½å¯¹æªç¥éè¯¯çåæ¡è°ç¨
 * @param {number} lid - åæ¡id
 */
