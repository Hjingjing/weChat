...
window.onload = function() {
    ...
    // 获取学生基本信息
    api.student.getInfo().then(function(data) {
        // 基本信息
        ...
        for (var i = 0; i < selectZone.options.length; i++) {
            if (selectZone.options[i].value == data.schoolZone) {
    ...
    document.getElementById('submit').onclick = function() {
        ...
        // 住宿及校园信息
        info.schoolZone = $('schoolZone').value;
        ...
        if (flag == 1) {
            api.student.postInfo(info, function(err, data) {
                if (err) {
